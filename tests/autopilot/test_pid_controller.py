...
def test_proportional():
    ...
    actual = pid.update(error=5.0, dt=1.0)
    expected = 2.0 * 5.0  # kp * (x_ref - x)
...
def test_integral():
    ...
    actual = pid.update(error=5.0, dt=1.0)
    expected = 1.0 * (5.0 + 0.0) * 0.5  # ki * (x_ref - x) * dt/2
    np.testing.assert_almost_equal(actual, expected)
    actual = pid.update(error=5.0, dt=1.0)
    expected += 1.0 * (5.0 + 5.0) * 0.5  # accumulate integral over 2 seconds
    np.testing.assert_almost_equal(actual, expected)
