...
from simulator.autopilot.navigation_v2.waypoints_manager import WaypointsManager
from simulator.common.constants import DEFAULT_HOME_COORDS
from simulator.sensors.state_handler import StateHandler


class Modes(Enum):
    STANDBY = 0  # aicraft off (only for initialization on ground)
    MANUAL = 1  # radio transmitter controls servo deflections
    STABILIZE = 2  # wing-leveling on stick release (transmitter control target attitude) <-- not recommended for planes
    FBWA = 3  # Fly-By-Wire-A: stabilized natural mode for planes (transmitter control changes on target attitude)
    FBWB = 4  # Fly-By-Wire-B: stabilized with altitude and airspeed control
    AUTO = 5  # bring control to waypoints manager
    ORBIT = 6  # do orbit on current position
    TAKE_OFF = 7  # take off mode to desired altitude, then changes to AUTO automatically
    LAND = 8 # alternative AUTO mode that switch to land control system 
    RTL = 9  # returns to home, launch or safe-point and orbit there at desired altitude
    FAILSAFE = 10  # control descend to ground
...
class Autopilot:
    def __init__(self, config: AutopilotConfiguration, filter: Filter) -> None:
        ...
        self.default_RTL_radius = 100.0

        self.state_filter = filter
        self.state_estimation: FilterEstimations = None
        
        self.home_coords = DEFAULT_HOME_COORDS
        self.launch_coords = DEFAULT_HOME_COORDS
        self.safepoint_coords = DEFAULT_HOME_COORDS
        ...
        ### AIRCRAFT NAVIGATION (mission to target path + vars)
        self.waypoints_manager = WaypointsManager(self.config)
