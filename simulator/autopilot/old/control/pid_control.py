...
class PID_control:
    def __init__(self, kp: float = 0.0, ki: float = 0.0, kd: float = 0.0, **kwargs) -> None:
        self.kp = kp
        self.ki = ki
        self.kd = kd
    ...
    @staticmethod
    def integrate(error: float, error_prev: float, intg_prev: float, dt: float) -> float:
        return intg_prev + dt / 2.0 * (error + error_prev)

    @staticmethod
    def differenciate(error: float, error_prev: float, diff_prev: float, dt: float, tau: float = None) -> float:
        if tau is None:
            tau = 5.0 * dt
        return ((2.0 * tau - dt) / (2.0 * tau + dt)) * diff_prev + (2.0 / (2.0 * tau + dt)) * (error - error_prev)
    ...
    def update(self, command: float, state: float, dt: float) -> float:
        ...
        if np.abs(self.ki) > EPS:
            error_intg = PID_control.integrate(error, self.prev_error, self.prev_intg, dt)
            control += self.ki * error_intg  # ki/s*e
            # self.prev_intg = error_intg

        # derivative term
        if np.abs(self.kd) > EPS:
            error_diff = PID_control.differenciate(error, self.prev_error, self.prev_diff, dt, self.tau)
